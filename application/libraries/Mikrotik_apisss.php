...
class Mikrotik_api
{
	...
	public function parse($read)
	{
		$ARRAY = $this->parseResponse4Smarty($read);
		$max = 0;
		$col = array();
		$rowh = array();
		$content = array();
		...
		foreach($ARRAY as $row)
		{
			$fild_array = array_keys($row);
			for($i=0; $i < $max;$i++)
			{
				if(in_array($col[$i],$fild_array))
					$content[$col[$i]] = $row[$col[$i]];
				else
					$content[$col[$i]] = NULL;
			}
			array_push($rowh,$content);
		}
		return json_encode($result = array ("result" => $rowh,
										  	"status" => "OK"));
	}
}